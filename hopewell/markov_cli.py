"""CLI handler for `hopewell query markov` (HW-0036).
...
def cmd_query_markov(args) -> int:
    """`hopewell query markov [--window W] [--no-singletons] [--top N] [--by K]`.
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        description="Markov / rework analytics (HW-0036). "
                    "Mirrors `hopewell query markov` (pending cli.py wiring).",
    )
