...
def score(project, version: str) -> Dict[str, Any]:
    ...
    cfg = load_config(project)
    from hopewell import release_confidence as rc
...
# Tuning knobs for the release-engineer agent + hopewell release CLI.
...
def write_default_config(project) -> Path:
    """Convenience for `hopewell release` onboarding. Idempotent."""
    path = config_path(project)
    if not path.is_file():
        path.parent.mkdir(parents=True, exist_ok=True)
