...
(rather than inline in the shell script) means:
  ...
  * Any Hopewell release that updates gate logic updates hook behaviour
    transparently (no need to re-run `hopewell hooks install`).
...
class GateResult:
    ...
    def format_for_hook(self) -> str:
        ...
        head = f"hopewell: [{self.gate}] {'OK' if self.ok else 'BLOCKED'} — {self.message}".rstrip(" —")
        lines = [head]
...
def check_drift(project_root: Path) -> GateResult:
    ...
    detail.append("            (b) `hopewell reconcile start <node>` to track a review,")
