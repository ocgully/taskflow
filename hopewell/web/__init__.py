"""Hopewell web UI — optional, requires the `[web]` extra.
...
def run(project_root: str = ".", port: int = 7420, open_browser: bool = False,
    ...
    Keeps `import hopewell.web` cheap on non-web installs; only when
    someone actually calls `run` do we pull in FastAPI/uvicorn.
    """
    from hopewell.web.server import run as _run
