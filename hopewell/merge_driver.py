...
Installed via `hopewell init` (or `hopewell merge-driver install`) which
...
# ---------------------------------------------------------------------------
# CLI entrypoint — `hopewell merge-driver jsonl %O %A %B`
# ---------------------------------------------------------------------------


def run_cli(args: List[str]) -> int:
    if len(args) < 3 or args[0] != "jsonl":
        print("usage: hopewell merge-driver jsonl <ancestor> <ours> <theirs>")
        return 1
