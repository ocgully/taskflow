"""CLI handler functions for `hopewell query cycle-time / quality / queue-staleness` (HW-0038).
...
from hopewell import cycle_time as ct_mod
...
def cmd_query_cycle_time(args) -> int:
    """`hopewell query cycle-time [<node_id>] [--component C] [--done-since ISO]`.
    ...
    try:
        ...
        print(f"hopewell: unknown node: {e}", file=sys.stderr)
        return 1
...
def cmd_query_quality(args) -> int:
    """`hopewell query quality <executor_id> [--since] [--all]`."""
    try:
        project = _project(args)
    except Exception as e:  # noqa: BLE001
        print(f"hopewell: {e}", file=sys.stderr)
    ...
    if not executor_id and not all_flag:
        print("hopewell: quality requires an executor id or --all", file=sys.stderr)
        return 1
...
def cmd_query_queue_staleness(args) -> int:
    """`hopewell query queue-staleness [--threshold DURATION]`."""
    try:
        project = _project(args)
    except Exception as e:  # noqa: BLE001
        print(f"hopewell: {e}", file=sys.stderr)
